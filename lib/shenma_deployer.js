...
module.exports = async function (args) {
	...
	if ('' === user_name || '' === token || channel.indexOf('shenma') === -1) {
		log.error(extName.concat("ShenMa submiter off."))
		return
	}
	...
	try {
		const {response} = await got("https://data.zhanzhang.sm.cn/push?site=abnerwei.com&user_name=" + user_name + "&resource_name=mip_add&token=" + token, {
			method: 'POST',
			headers: {
